...
const assertStoryEngine = (engine) => {
  if (!engine || typeof engine.createScenario !== "function" || typeof engine.saveState !== "function") {
    throw new TypeError("initializeChapter requires a compatible PhysicsEngine instance.");
  }
  return engine;
};

export function initializeChapter(engine, index) {
  assertStoryEngine(engine);
  index = validateChapterIndex(index);
  ...
  // The chapter's initial state must include the chapter bodies themselves.
  // createScenario() records a pre-chapter baseline, so replace it with the
  // fully initialized chapter state to keep reset/seek/history deterministic.
  engine.initialState = engine.saveState();
  engine.history = [];
