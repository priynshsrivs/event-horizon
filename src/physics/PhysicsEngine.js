...
export const BODY_PRESETS = {
  asteroid: {
    ...
    radius: 1e-7,
    temperature: 190,
    metadata: { color: "#938e85", visualSize: 0.0175 },
  ...
  planet: {
    ...
    temperature: 288,
    metadata: { color: "#70b3c7", visualSize: 0.0425 },
  },
  "rogue planet": {
    mass: 9.54e-4,
    radius: 0.00047,
    temperature: 70,
    metadata: { color: "#6d87bb", visualSize: 0.68 },
  },
  comet: {
    mass: 3e-17,
    radius: 2e-8,
    ...
    metadata: { color: "#b2ebef", visualSize: 0.0175 },
  },
  star: {
    mass: 1,
    ...
    metadata: { color: "#ffc77a", visualSize: 0.1, solarActivity: 1 },
  },
  "red giant": {
    mass: 2,
    radius: 0.2,
    ...
    temperature: 3500,
    composition: ["Hydrogen", "Helium"],
    metadata: { color: "#ff7754", visualSize: 2.4 },
  },
  "red supergiant": {
    mass: 15,
    radius: 3,
    ...
    temperature: 3600,
    composition: ["Hydrogen", "Helium"],
    metadata: { color: "#fa583c", visualSize: 4 },
  },
  "white dwarf": {
    mass: 0.6,
    radius: 0.00005,
    ...
    metadata: { color: "#d9edff", visualSize: 0.17 },
  },
  "brown dwarf": {
    mass: 0.04,
    radius: 0.00047,
    ...
    temperature: 1400,
    composition: ["Hydrogen", "Helium"],
    metadata: { color: "#a36043", visualSize: 0.68 },
  },
  "neutron star": {
    mass: 1.4,
    radius: 12000 / AU_M,
    ...
    metadata: { color: "#abe6ff", visualSize: 0.003 },
  },
  magnetar: {
    mass: 1.7,
    radius: 12000 / AU_M,
  ...
  "black hole": {
    mass: 500,
    radius: (2 * G_SI * 500 * SOLAR_MASS_KG) / C_M_PER_S ** 2 / AU_M,
    temperature: 0,
    composition: ["Compact gravitational source"],
    metadata: { color: "#e8a874", visualSize: 2.72, kerrSpin: 0.6 },
  },
  wormhole: {
    mass: 1e-15,
    radius: 0.15,
    collisionMode: "ignore",
    metadata: { color: "#ad9fff", visualSize: 0.085 },
  },
  "dark matter halo": {
    mass: 3,
    radius: 5,
    ...
    metadata: { color: "#8a7fcc", visualSize: 1.1, softening: 3 },
  },
};
...
export class PhysicsEngine {
  constructor(options = {}) {
    ...
    this.fixedDt = clamp(finite(options.fixedDt, DEFAULT_FIXED_DT), MIN_FIXED_DT, 1 / 2048);
  ...
  setTimeScale(value) {
    ...
    // update() still caps work per render tick so extreme values degrade by
    // dropping excess simulated time rather than destabilizing the integrator.
    this.timeScale = clamp(finite(value, 0.04), -MAX_TIME_SCALE, MAX_TIME_SCALE);
    this.accumulator = 0;
  }
  ...
  getPortalBasis(portal) {
    let normal = Vector3.from(portal?.metadata?.portalNormal || [0, 0, 1]);
    if (normal.lengthSq() < 1e-12) normal.set(0, 0, 1);
    normal.normalize();

    let up = Vector3.from(portal?.metadata?.portalUp || [0, 1, 0]);
    if (up.lengthSq() < 1e-12) up.set(0, 1, 0);
    up.normalize();
    // Gram-Schmidt: force the supplied up vector to be orthogonal to the portal normal.
    up.addScaledVector(normal, -up.dot(normal));
    if (up.lengthSq() < 1e-12) {
      up = Math.abs(normal.y) < 0.9
        ? new Vector3(0, 1, 0)
        : new Vector3(1, 0, 0);
  ...
  transformThroughPortal(vector, source, target) {
    const sourceBasis = this.getPortalBasis(source);
    const targetBasis = this.getPortalBasis(target);
    const localRight = vector.dot(sourceBasis.right);
    const localUp = vector.dot(sourceBasis.up);
    const localNormal = vector.dot(sourceBasis.normal);

    // A traveller exits the destination portal through its outward side, so
    // the destination normal is flipped. Tangential components retain their
    // orientation in the portal-local frame.
    return targetBasis.right
      .clone()
      .multiplyScalar(localRight)
      .addScaledVector(targetBasis.up, localUp)
      .addScaledVector(targetBasis.normal, -localNormal);
  }
    ...
    for (const source of [...this.bodies]) {
      ...
      if (source.type === "wormhole") {
        ...
        for (const b of [...this.bodies]) {
          ...
          const velocityBefore = b.velocity.clone();
          const transformedVelocity = this.transformThroughPortal(
            velocityBefore,
            source,
            target,
          );
          const exitDirection = transformedVelocity.length()
            ? transformedVelocity.clone().normalize()
            : this.getPortalBasis(target).normal.clone().negate();

          b.position
            .copy(target.position)
            .addScaledVector(exitDirection, target.radius * 1.3 + b.radius);
          b.velocity.copy(transformedVelocity);
          b._previous?.copy(b.position);
          b.metadata.portalCooldownUntil = this.time + 0.02;
          this.emit("wormholeTransit", {
            body: b,
            source,
            target,
            velocityBefore,
            velocityAfter: transformedVelocity.clone(),
          });
        }
      }
    }
  ...
  updateEnvironment(dt) {
    ...
    for (const body of [...sources]) {
      ...
      if (strongest && this.settings.tides) {
        ...
        const tidalPrimary = this.getBody(strongest.primaryId);
        const contactDistance = body.radius + (tidalPrimary?.radius || 0);
        const outsideContact =
          !!tidalPrimary &&
          strongest.distance > contactDistance * (1 + 1e-6);
        if (
          strongest.distance < strongest.rocheLimit &&
          outsideContact &&
          !body.metadata.fragment &&
          (body.metadata.collisionCooldown || 0) <= this.time &&
          this.getBody(body.id)
        ) {
          body.metadata.rocheDisrupted = true;
  ...
  restoreState(state, { preservePlayback = false } = {}) {
    ...
    this.fixedDt = clamp(
      finite(state.fixedDt, DEFAULT_FIXED_DT),
      MIN_FIXED_DT,
      1 / 2048,
    );
    ...
    if (!preservePlayback) {
      this.timeScale = clamp(finite(state.timeScale, 0.04), -MAX_TIME_SCALE, MAX_TIME_SCALE);
  ...
  spawnWormholePair(center = new Vector3(3, 0, 0)) {
    ...
    const firstPosition = Vector3.from(center).add(new Vector3(-1, 0, -1));
    const secondPosition = Vector3.from(center).add(new Vector3(1, 0, 1));
    ...
    return [
      ...
      this.spawnBody("wormhole", {
        ...
        metadata: {
          wormholeLink: link,
          portalNormal: axis.clone().negate().toArray(),
          portalUp: [0, 1, 0],
        },
      }),
    ];
