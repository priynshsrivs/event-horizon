...
function textureCandidates(name) {
  const extensions = TEXTURE_EXTENSIONS[name] || ["jpg", "jpeg", "png", "webp"];
  return extensions.map((ext) => `/textures/${name}.${ext}`);
}
    ...
    <>
      ...
      {clouds && (
        <mesh
          ...
          rotation={[0, 0.02, 0]}
        >
          <sphereGeometry args={[radius * 1.012, settings.quality === "medium" ? 40 : 56, settings.quality === "medium" ? 28 : 40]} />
          <meshStandardMaterial
            map={clouds}
            alphaMap={clouds}
            transparent
  ...
  return (
    <>
      <Canvas
        ...
        camera={{ position: [13, 17, 21], fov: 43, near: 0.05, far: 600 }}
        dpr={
