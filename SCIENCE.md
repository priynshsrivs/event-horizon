...
## Web Worker architecture and trade-offs

For applications requiring background integration, the `src/physics/worker/` subsystem exposes `physicsWorker.js` and `PhysicsWorkerClient.js`. 

- **Trade-off Analysis**: For systems capped at $N \le 128$ bodies, Velocity Verlet integration on the main thread takes less than $0.38\text{ ms}$ per 1,000 steps (or $< 0.005\text{ ms}$ per animation frame). By comparison, structured cloning via `postMessage` between a Web Worker and the main thread for 128 bodies typically requires $0.5\text{ to }1.0\text{ ms}$ per frame. Therefore, running physics on the main thread is optimal for low-latency Three.js render synchronization at current body limits, while the worker architecture provides an isolated option for background computation, long-horizon trajectory precomputation, or heavy batch simulation without blocking UI thread event loops.
